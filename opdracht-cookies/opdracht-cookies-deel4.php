...
$input          =   json_decode(file_get_contents('gegevens2.txt'), true);
$message        =   '';
$Authenticated  =   false;
...
if (isset( $_POST['submit'])) 
    {
        ...
        foreach ($input as $key => $gegevens) 
        {
        
            if ($_POST['username'] == $gegevens['username'] &&
                $_POST['paswoord'] == $gegevens['paswoord']) 
            {
                ...
                setcookie('klopt', $key, $tijd);
                //belangrijk om de value van 'setcookie' op $key te zetten, want anders wordt de value van $_COOKIE['klopt'] = '1', wat gelijk staat aan de tweede array, en hier dus 'Floor' is.
...
{
    $Authenticated  =   true;
    $userId         =   $_COOKIE['klopt'];
    $welkom         =   'Hallo ' . ucfirst($input[$userId]['username']) . ', fijn dat je er weer bij bent!';
    //var_dump($_COOKIE);
}
